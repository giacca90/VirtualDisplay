...
// Negociación necesaria → crea offer
static void on_negotiation_needed(GstElement *webrtcbin, gpointer _user_data) {
	g_print("📞 Negociación requerida → creando offer\n");
    GstPromise *promise =
...
static void on_ws_message(	SoupWebsocketConnection *conn, 
	...
	if (g_strcmp0(msg_type, "ready") == 0) {
		g_print("📩 Recibido 'start-negotiation' → creando offer...\n");

		// Lanza la negociación
...
static void on_ws_connected(SoupSession *session, GAsyncResult *res, gpointer _user_data) {
    ...
    // lanzar pipeline
	g_print("🎬 Lanzando pipeline GStreamer en estado PLAYING\n");
